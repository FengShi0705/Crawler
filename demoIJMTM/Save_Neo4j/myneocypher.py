...
def CreateUnique_Node_Word(cypher,words,wids): #OK no problem!!!

    for i,word in enumerate(words):
        cypher.execute(
            "merge (:Word {name:{n}, wid:{w}})",n=word,w=wids[i]
        )
...
def CreateUnique_Node_Paper(cypher,pid,URL,title,Journal,date,Keytexts): #OK no problem!!!




    cypher.execute(
            "merge (:Paper {URL:{URL},pid:{pid},title:{title},Journal:{Journal},date:{date},Keytexts:{Keytexts}})",
            URL=URL,pid=pid,title=title,Journal=Journal,date=date,Keytexts=Keytexts
...
def CreateUnique_Node_Author(cypher,authors,aids): # OK no problem!!!

    for i,author in enumerate(authors):
        cypher.execute(
            "merge (:Author {name:{n}, aid:{a}})",n=author,a=aids[i]
        )
